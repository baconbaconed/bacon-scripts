...
local trueBanned = {}
local seenTracks = {}
local replacingTargets = {}
local replacementFrozen = false
local scriptTracks = {}
local keybinds = {}
local replacements = {}
local favorites = {}
...
local keybindGroupsFile = "bacon_logger_groups.json"
local keybindGroups = {}
local animLastFired = {}
local animFireIntervals = {}
local loopingAnims = {}

local function dumpGroups()
	if writefile then
		writefile(keybindGroupsFile, HttpService:JSONEncode(keybindGroups))
	end
end
local function loadGroups()
	if readfile and isfile and isfile(keybindGroupsFile) then
		local ok, data = pcall(function() return HttpService:JSONDecode(readfile(keybindGroupsFile)) end)
		if ok and type(data) == "table" then keybindGroups = data end
	end
end
...
notifLabel.Size = UDim2.new(0, 400, 0, 32)
notifLabel.AnchorPoint = Vector2.new(0.5, 0)
notifLabel.Position = UDim2.new(0.5, 0, 0, 8)
notifLabel.BackgroundColor3 = Color3.fromRGB(80, 20, 10)
notifLabel.BackgroundTransparency = 1
notifLabel.TextColor3 = Color3.fromRGB(255, 120, 80)
...
do
	local c = Instance.new("UICorner"); c.CornerRadius = UDim.new(0, 5); c.Parent = notifLabel
	local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(180, 60, 30); s.Thickness = 1; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border; s.Parent = notifLabel
end
...
sideList.Position = UDim2.new(0, 5, 0, 58)
sideList.Size = UDim2.new(1, -10, 1, -93)
...
local groupNameBox = Instance.new("TextBox")
groupNameBox.PlaceholderText = "group name..."
groupNameBox.Text = ""
groupNameBox.Size = UDim2.new(0.55, -8, 0, 22)
groupNameBox.Position = UDim2.new(0, 5, 0, 33)
groupNameBox.BackgroundColor3 = Color3.fromRGB(12, 16, 22)
groupNameBox.TextColor3 = Color3.new(1, 1, 1)
groupNameBox.PlaceholderColor3 = Color3.fromRGB(0, 100, 80)
groupNameBox.Font = Enum.Font.Code
groupNameBox.TextSize = 11
groupNameBox.BorderSizePixel = 0
groupNameBox.ZIndex = 3
groupNameBox.Parent = sideFrame
mkCorner(groupNameBox, 3)
mkStroke(groupNameBox, Color3.fromRGB(0, 70, 80), 1)

local saveGroupBtn = Instance.new("TextButton")
saveGroupBtn.Text = "Save"
saveGroupBtn.Size = UDim2.new(0.22, -4, 0, 22)
saveGroupBtn.Position = UDim2.new(0.55, 2, 0, 33)
saveGroupBtn.BackgroundColor3 = Color3.fromRGB(0, 60, 70)
saveGroupBtn.TextColor3 = Color3.fromRGB(0, 220, 170)
saveGroupBtn.Font = Enum.Font.Code
saveGroupBtn.TextSize = 11
saveGroupBtn.BorderSizePixel = 0
saveGroupBtn.ZIndex = 3
saveGroupBtn.Parent = sideFrame
mkCorner(saveGroupBtn, 3)
mkStroke(saveGroupBtn, Color3.fromRGB(0, 110, 120), 1)

local loadGroupBtn = Instance.new("TextButton")
loadGroupBtn.Text = "Load"
loadGroupBtn.Size = UDim2.new(0.22, -4, 0, 22)
loadGroupBtn.Position = UDim2.new(0.77, 2, 0, 33)
loadGroupBtn.BackgroundColor3 = Color3.fromRGB(30, 20, 50)
loadGroupBtn.TextColor3 = Color3.fromRGB(180, 150, 255)
loadGroupBtn.Font = Enum.Font.Code
loadGroupBtn.TextSize = 11
loadGroupBtn.BorderSizePixel = 0
loadGroupBtn.ZIndex = 3
loadGroupBtn.Parent = sideFrame
mkCorner(loadGroupBtn, 3)
mkStroke(loadGroupBtn, Color3.fromRGB(90, 60, 180), 1)

local groupDropdown = Instance.new("Frame")
groupDropdown.Size = UDim2.new(1, -10, 0, 0)
groupDropdown.Position = UDim2.new(0, 5, 0, 57)
groupDropdown.BackgroundColor3 = Color3.fromRGB(14, 18, 24)
groupDropdown.BorderSizePixel = 0
groupDropdown.ZIndex = 20
groupDropdown.ClipsDescendants = true
groupDropdown.Visible = false
groupDropdown.Parent = sideFrame
mkCorner(groupDropdown, 4)
mkStroke(groupDropdown, Color3.fromRGB(90, 60, 180), 1)
do
	local groupDropLayout = Instance.new("UIListLayout")
	groupDropLayout.Padding = UDim.new(0, 1)
	groupDropLayout.Parent = groupDropdown
end

local function refreshGroupDropdown()
	for _, c in ipairs(groupDropdown:GetChildren()) do
		if c:IsA("TextButton") then c:Destroy() end
	end
	local count = 0
	for name, _ in pairs(keybindGroups) do
		count += 1
		...
		btn.ZIndex = 21
		btn.Parent = groupDropdown
		table.insert(connections, btn.MouseButton1Click:Connect(function()
			local data = keybindGroups[name]
	...
	groupDropdown.Size = UDim2.new(1, -10, 0, count * 23)
end

table.insert(connections, saveGroupBtn.MouseButton1Click:Connect(function()
	local name = groupNameBox.Text:match("^%s*(.-)%s*$")
	...
	keybindGroups[name] = data
	dumpGroups()
	saveGroupBtn.Text = "Saved!"
	task.delay(1.5, function() saveGroupBtn.Text = "Save" end)
end))

table.insert(connections, loadGroupBtn.MouseButton1Click:Connect(function()
	refreshGroupDropdown()
	groupDropdown.Visible = not groupDropdown.Visible
end))
...
table.insert(connections, sideMinimize.MouseButton1Click:Connect(function()
	...
	if sideMinimized then
		...
		saveGroupBtn.Visible = false
		loadGroupBtn.Visible = false
		groupDropdown.Visible = false
		sideMinimize.Text = "+"
	else
		sideFrame.Size = UDim2.new(0, 200, 0, 480)
		...
		saveGroupBtn.Visible = true
		loadGroupBtn.Visible = true
		sideMinimize.Text = "-"
	end
end))
...
local function addReplUI(id, targetId, speed, loop, enabled)
	...
	local function updateRep()
		...
		replacements[id] = { targetId = targetL.Text, speed = tonumber(speedL.Text) or 1, loop = loop, enabled = enabled }
		dumpCfg()
	end
	...
	table.insert(connections, enableT.MouseButton1Click:Connect(function()
		enabled = not enabled
		enableT.Text = enabled and "Enabled: ON" or "Enabled: OFF"
		enableT.BackgroundColor3 = enabled and Color3.fromRGB(0, 55, 0) or Color3.fromRGB(55, 0, 0)
		updateRep()
...
local function checkLoopDetect(id, key)
	...
	if animator then
		for _, t in pairs(animator:GetPlayingAnimationTracks()) do
			if scriptTracks[t] and grabId(t.Animation.AnimationId) == id then
				isScriptAnim = true
				break
			end
		end
	...
	local now = tick()
	if animLastFired[key] then
		local interval = now - animLastFired[key]
		if interval > 0.3 and interval < 30 then
			if not animFireIntervals[key] then animFireIntervals[key] = {} end
			table.insert(animFireIntervals[key], interval)
			if #animFireIntervals[key] > 5 then table.remove(animFireIntervals[key], 1) end
			if #animFireIntervals[key] >= 3 then
				local avg = 0
				for _, v in ipairs(animFireIntervals[key]) do avg += v end
				avg = avg / #animFireIntervals[key]
				local consistent = true
				for _, v in ipairs(animFireIntervals[key]) do
					if math.abs(v - avg) > avg * 0.3 then consistent = false break end
				end
				if consistent then
					loopingAnims[key] = true
...
local function logIt(id, name, playerName, priority)
	...
	if not animCounts[key] then
		...
		local entry = animFrames[key]
		if entry then
			local prefix = loopingAnims[key] and "\xe2\x86\xbb " or ""
			entry.Text = prefix .. "(" .. animCounts[key] .. ") [" .. (priority or "N/A") .. "] " .. id .. " | " .. entryName .. " @" .. animTimestamps[key]
		end
	end
end
...
local function trackSeen(track, playerName)
	...
	if banned[id] or trueBanned[id] then stopTrack(track); return end
	...
	for _, p in pairs(Players:GetPlayers()) do
		...
		if animator then
			...
			for _, track in pairs(animator:GetPlayingAnimationTracks()) do
				...
				if isLocal and tid and replacements[tid] and replacements[tid].enabled ~= false and (replacingTargets[replacements[tid].targetId] or replacementFrozen) then
					track:AdjustWeight(0, 0)
					track:AdjustSpeed(0)
					track:Stop(0)
					continue
